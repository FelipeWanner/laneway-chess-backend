...
const validateEnPassant = (from, to, color, boardState, lastMove) => {
  ...
  const validateKnightMove = (from, to, boardState) => {
    ...
    const isDestinationOccupiedByAlly = destinationPiece && destinationPiece.color === boardState[fromRow][fromCol].color;
  
    if (isLShapeMove && !isDestinationOccupiedByAlly) {
      return true;
    } else {
      //console.log("Invalid knight move: Cannot capture an ally piece or invalid move shape");
      return false;
    }
  };
...
const isMoveValid = (piece, from, to, boardState, lastMove, hasKingMoved, hasRookMoved) => {
  if (!isWithinBoard(to)) {
    //console.log("Move out of board boundaries");
    return false;
  }
  console.log("Passed hasRookMoved:", hasRookMoved);
